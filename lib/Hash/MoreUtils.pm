...
If no C<LIST> is given it returns an empty C<HASHREF>.
...
As C<slice_missing> but only checks for missing keys from C<MAP> in C<HASHREF>.

If no C<MAP> is given, C<slice_missing> will be used only on C<HASHREF>.

=head2 C<slice_notdef_map> HASHREF[, MAP]

As C<slice_notdef> but only checks for undefined keys from C<MAP> in C<HASHREF>.
